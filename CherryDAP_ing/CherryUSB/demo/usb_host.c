...
#include "usbh_core.h"
#include "FreeRTOS.h"
usb_osal_thread_t AudioTaskTCB;
uint8_t AudioReadBuffer[1024];

static void audio_iso_in_callback(void *arg, int packs)
{
    printf("rc packs %d", packs);
}


static void usbh_audio_thread(CONFIG_USB_OSAL_THREAD_SET_ARGV)
{
    int ret;
    struct usbh_urb urb;
    struct usbh_audio *audio_class = (struct usbh_audio *)CONFIG_USB_OSAL_THREAD_GET_ARGV;
    uint8_t *buf = pvPortMalloc(audio_class->isoin_mps); // 缓冲区大小 ≥ 最大包长
    ret = usbh_audio_open(audio_class, "mic", 16000, 16);

    urb.hport = audio_class->hport;
    urb.ep = audio_class->isoin;      // 设备的 ISO IN 端点
    urb.setup = NULL;                 // 非控制传输
    urb.transfer_buffer = buf;
    urb.transfer_buffer_length = audio_class->isoin_mps;
    urb.timeout = 0;                   // ISO 不需要超时
    urb.complete = audio_iso_in_callback; // 回调处理
    urb.arg = NULL;
    urb.interval = USBH_GET_URB_INTERVAL(audio_class->isoin->bInterval,
                                     audio_class->hport->speed);

    usbh_submit_urb(&urb); // 提交接收请求
    if (ret < 0) {
        USB_LOG_RAW("audio open error,ret:%d\r\n", ret);
    }
    while(1)
    {
...
{
    AudioTaskTCB = usb_osal_thread_create("usbh_msc", 2048, CONFIG_USBHOST_PSC_PRIO + 1, usbh_audio_thread, audio_class);
}
...
void usbh_audio_stop(struct usbh_audio *audio_class)
{
    ...
    ret = usbh_audio_close(audio_class, "mic");
    
    if (ret < 0) {
        USB_LOG_RAW("audio close error,ret:%d\r\n", ret);
    }
    usb_osal_thread_delete(AudioTaskTCB);
}
#endif
