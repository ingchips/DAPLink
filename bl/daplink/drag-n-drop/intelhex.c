    ...
    *bin_buf_cnt = (uint32_t)0;
    ...
    while (hex_blob != end) {
        switch ((uint8_t)(*hex_blob)) {
            ...
            // decoding lines
            default:
                if (!is_hex_digit(*hex_blob) || (idx >= sizeof(line.buf))) {
                    status = HEX_PARSE_FAILURE;
                    goto hex_parser_exit;
                }
                ...
                if (low_nibble) {
                    ...
                    idx++;
                    if ((idx == 1U) && (line.byte_count > sizeof(line.data))) {
                        status = HEX_PARSE_FAILURE;
                        goto hex_parser_exit;
                    }
                                ...
                                switch (line.record_type) {
                                    ...
                                    case CUSTOM_DATA_RECORD:
                                        if (binary_version == 0 || binary_version == board_id_hex_default || binary_version == board_id_hex) {
                                            ...
                                            // Flush the current output before loading a record that would overflow it.
                                            if (((uint32_t)(*bin_buf_cnt) + line.byte_count) > bin_buf_size) {
