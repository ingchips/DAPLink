...
#ifndef MSC_ENDPOINT
bool USBD_MSC_MediaReady = false;
bool USBD_MSC_ReadOnly = false;
...
// this func is a callback from the file system
// this func in usb msc file.
void msc_write_handler(uint32_t sector,uint8_t *buffer, uint32_t num_of_sectors)
{
    sync_assert_usb_thread();
    ...
    vfs_write(sector, buffer, num_of_sectors);
    if (TRASNFER_FINISHED == file_transfer_state.transfer_state) {
        return;
    }
    file_data_handler(sector, buffer, num_of_sectors);
}
