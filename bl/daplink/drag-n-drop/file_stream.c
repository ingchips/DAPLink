...
{
    // printf("hex detect\n");
   return (size > 8) && (data[0] == ':') &&
          ((data[8] == '0') || (data[8] == '2') || (data[8] == '3') ||
           (data[8] == '4') || (data[8] == '5'));
}
...
static error_t write_hex(void *state, const uint8_t *data, uint32_t size)
{
    ...
    while (1) {
        ...
        // printf("hex_state->bin_buffer size %x size 0x%x block_amt_parsed %x bin_start_address %x bin_buf_written %x\n",sizeof(hex_state->bin_buffer), size, block_amt_parsed, bin_start_address,bin_buf_written);

        // the entire block of hex was decoded. This is a simple state
        if (HEX_PARSE_OK == parse_status) {
            // printf("hex HEX_PARSE_OK\n");
            ...
            // printf("11 hex HEX_PARSE_UNALIGNED\n");
            if (bin_buf_written > 0) {
                // printf ("need write data\n");
                status = flash_decoder_write(bin_start_address, hex_state->bin_buffer, bin_buf_written);
...
//            util_assert(HEX_PARSE_UNINIT != parse_status);
    ...
    // printf("break and return status\n");
