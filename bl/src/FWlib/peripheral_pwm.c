...
void IR_SetPeriodAndDuty(uint8_t channel, uint8_t carry_channel, uint32_t period, uint32_t high)
{
    if(carry_channel)
    {
        APB_PWM->STEPChannels[channel].step1 = period;
        APB_PWM->Channels[channel].DZoneTh = high;
    }
    else
    {
        APB_PWM->Channels[channel].PeraTh = period;
        APB_PWM->Channels[channel].HighTh = high;
    }
}
...
void IR_WriteCarrierReloadData(uint8_t channel, uint32_t data)
{
    uint32_t  reg;
    reg = data &  ((1<<20) - 1);
    APB_PWM->Channels[channel].DmaData = reg;
